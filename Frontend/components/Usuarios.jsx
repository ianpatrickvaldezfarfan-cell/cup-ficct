...
admin.nuevo,admin@ficct.edu.bo,pass123,1`}
                            ...
                            <table className="table mb-0" style={{ borderCollapse: 'separate', borderSpacing: 0 }}>
                                <thead>
                                    <tr style={{ background: '#1a3a6b' }}>
                                        {['#','Username','Correo','Rol','Estado','Acciones'].map(h => (
                                ...
                                <tbody>
                                    {usuarios.length === 0 ? (
                                        <tr><td colSpan={6} style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>Sin resultados</td></tr>
                                    ) : usuarios.map((u, i) => (
            ...
            {mostrarModal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1050, padding: '1rem' }}>
                    <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 480, boxShadow: '0 25px 80px rgba(0,0,0,0.3)', overflow: 'hidden', animation: 'fadeSlideUp 0.25s ease' }}>
