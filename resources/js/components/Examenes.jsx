 ...
 *
 * FLUJO 1 - Administrador/Docente registra notas:
 * Mensaje 1: buscarPostulante(ci)
 *   → handleBuscar() con input del CI
 *
 * Mensaje 1.1: obtenerPostulantePorCI(ci)
 *   → GET /api/examenes/postulante/{ci}
 *
 * Mensaje 1.4: mostrarMaterias(Computacion, Matematicas, Ingles, Fisica)
 *   → setPostulanteActual(data) → render tabla de materias
 *
 * Mensaje 1.5: registrarNotas(postulacion_id, materia_id, nota1, nota2, nota3)
 *   → handleGuardarNotas() abre modal de registro
 *
 * Mensaje 1.6: procesarNotas()
 *   → POST /api/examenes o PUT /api/examenes/{id}
 *
 * Mensaje 1.13: mostrarResultado(promedio, estado_materia,
 *              promedio_global, estado_admision)
 *   → Actualiza tabla con nuevas notas y estados
 *
 * FLUJO 2 - Postulante consulta:
 * Mensaje 2: consultarNotas(ci)
 *   → Automático al cargar PanelPostulante
 *
 * Mensaje 2.4: mostrarCalificaciones()
 *   → Render tabla de notas en PanelPostulante
 */
        ...
        <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
            ...
            {modal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1050, padding: '1rem' }}>
                    <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 440, boxShadow: '0 25px 80px rgba(0,0,0,0.3)', overflow: 'hidden', maxHeight: '90vh', overflowY: 'auto' }}>
